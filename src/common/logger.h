...
void initLogger()
{
    ...
    int max_log_file_size = 50 * 1024 * 1024;
    int max_log_file_num = 3;
    ...
    try
    {
        ...
        auto info_sink =
            std::make_shared<spdlog::sinks::rotating_file_sink_mt>(log_dir + "/janus.log", max_log_file_size, max_log_file_num);
        info_sink->set_level(global_level);

        // --- Warn Sink: 存放 Warn 等级 ---
        auto warn_sink =
            std::make_shared<spdlog::sinks::rotating_file_sink_mt>(log_dir + "/janus.warn", max_log_file_size, max_log_file_num);
        warn_sink->set_level(spdlog::level::warn);

        // --- Error Sink: 只存放 Error ---
        auto error_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(log_dir + "/janus.error",
